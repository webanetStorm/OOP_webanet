...
{
	if ( _pParent )
		this->_pParent->_childs.push_back( this );
}
...
{
	if ( this->_pParent )
		for ( auto pChild : this->_pParent->_childs )
...
Base* Base::FindOnBranch( string name )
{
	...
	while ( !queue.empty() )
	{
		...
		for ( auto pChild : queue.front()->_childs )
			queue.push( pChild );
...
bool Base::SetNewParent( Base* pNewParent )
{
	if ( pNewParent && pNewParent != this && !pNewParent->FindOnBranch( this->_name ) && this->_pParent )
	{
		...
		this->_pParent->_childs.erase( find( this->_pParent->_childs.begin(), this->_pParent->_childs.end(), this ) );
		pNewParent->_childs.push_back( this );
...
void Base::DeleteChildByName( string name )
{
	...
	if ( pChild )
	{
		this->_childs.erase( find( this->_childs.begin(), this->_childs.end(), pChild ) );
...
void Base::DeleteConnection( TYPE_SIGNAL pSignal, Base* pTarget, TYPE_HANDLER pHandler )
{
	...
	{
		return connection->Signal == pSignal && connection->Target == pTarget && connection->Handler == pHandler;
	} );
